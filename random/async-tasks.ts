...
const printNumbersTimeout = (from: number, to: number) => {
    ...
    const timeout = () => {
        console.log(cur);
        if (cur < to) setTimeout(timeout, 1000);
        cur++;
    }
...
const getResults = async (funcsArray: (() => Promise<string>)[]) => {
  const results = await Promise.all(
    funcsArray.map(async (func) => await func())
  ).then((val) => val);
...
const results = async () =>
    await getResults([asyncFunc1, asyncFunc2, asyncFunc3]);
...
const resolvePromisesSeq = async (functions: (() => Promise<string>)[]) => {
  ...
  const sequenceResults = async () =>
    await resolvePromisesSeq([asyncFunc1, asyncFunc2, asyncFunc3]);
...
const promiseAll = async (functions: Promise<string>[]) => {
  return new Promise((res, rej) => {
    const results: string[] = [];
...
promiseAll([
  fakePropmise("one", 100),
  fakePropmise("second", 500),
  fakePropmise("third", 300, true)
])
...
const promiseRace = async (functions: Promise<string>[]) => {
  return new Promise((res, rej) => {
    functions.forEach((func, index) => {
      func
        ...
        .catch((err) => rej(`rejected ${index + 1}`));
    });
  });
};

promiseRace([
  fakePropmise("one", 400),
  fakePropmise("second", 500),
  fakePropmise("third", 300, true)
])
  .then((val) => console.log(val))
...
const mapPromiseSequential = async (functions: Promise<string>[]) => {
  const results: string[] = [];
...
console.log(mapPromiseSequential([
  fakePropmise("one", 400),
  fakePropmise("second", 500),
  fakePropmise("third", 300)
]))
